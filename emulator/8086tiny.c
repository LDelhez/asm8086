...
void my_interrupt(unsigned char interrupt_num) {
	switch (interrupt_num) {
		case 0x00: // Print byte as hex
			printf("%#x\r\n", regs8[REG_BL]);
			break;
		case 0x01: // Print word as hex
			printf("%#x\r\n", regs16[REG_BX]);
			break;
		case 0x02: // Print byte as char
			printf("%c\r\n", regs8[REG_BL]);
			break;
		case 0x03: // Print byte as uint
			printf("%u\r\n", regs8[REG_BL]);
			break;
		case 0x04: // Print word as uint
			printf("%u\r\n", regs16[REG_BX]);
			break;
		case 0x05: // Print string
			printf("%s\r\n", mem + regs16[REG_DS] * 16 + regs16[REG_BX]);
			break;
		case 0x06: // Put char
			printf("%c", regs8[REG_AL]);
			fflush(stdout);
			break;
		case 0x07:; // Shift 4 <- ax
			unsigned short ax = regs16[REG_BX];
			ax = ax << 4;
			regs16[REG_BX] = ax;
			break;
		case 0x08:; // Get prog.com size -> bx
			FILE* fp1 = fopen("prog.com", "r");
			fseek(fp1, 0L, SEEK_END);
			unsigned short filelength = ftell(fp1);
			fclose(fp1);
			regs16[REG_BX] = filelength;
		 	break;
		case 0x09:; // Load prog.com on stack
			FILE* fp2 = fopen("prog.com", "r");
			fseek(fp2, 0L, SEEK_END);
			unsigned short filesize = ftell(fp2);
			fseek(fp2, 0L, SEEK_SET);
			fread(mem + 16 * regs16[REG_SS] + regs16[REG_SP], sizeof(unsigned char), filesize, fp2);
			fclose(fp2);
			break;
		}
}
...
char pc_interrupt(unsigned char interrupt_num)
{
	...
	if (interrupt_num == 0xF0) {
		my_interrupt(regs8[REG_AH]);
		return regs8[FLAG_TF] = regs8[FLAG_IF] = 0;
	}

	// Debug
	if (interrupt_num == 0xFF) {
		printf("ax: %#x\r\n", regs16[REG_AX]);
		return regs8[FLAG_TF] = regs8[FLAG_IF] = 0;
	}
